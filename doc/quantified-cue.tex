...
\textbf{124,523 assertions} and records the result in \code{checks/results.json}.
...
These are executable reference models, not tests of the Go evaluator or proofs
for infinite semantic universes. Both scripts use explicit checks that remain
enabled under Python's optimization option and write deterministic reports;
\code{checks/README.md} gives the commands to reproduce them.
The additional \code{checks/check_residual.py} executes \textbf{133,688 assertions}
