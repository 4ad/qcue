...
The following families describe finite reference checks for the algebraic laws
and counterexamples. A suitable model enumerates four data values, their sixteen
subsets, and 625 partial function tables, with a separate rejection outcome.
This checkout does not include the reference-model scripts or their result
files; no executed assertion count is claimed here.
...
Finite checks for the local kernel of \cref{ch:residual} should additionally
cover preservation of completion sets under bounded propagation, validation
of grounded arithmetic cycles, persistence of refutations, retention of guards
and existential alternatives, and quantified-function counterexamples.
Such reference checks supplement rather than establish correctness of the Go
implementation. Executable implementation regressions are indexed in
\code{cue/testdata/quantified/README.md}; supported fragments and checking limits
are documented in \code{doc/quantified-cue-implementation.md}.
