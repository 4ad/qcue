...
func (c *converter) field(f *ast.Field) doc {
	...
	var preVal doc
	if HasDocComment(value) || valNeedsLeadingBreak(value) || len(slots.suffix) > 0 {
...
func (c *converter) fieldRow(chain []*ast.Field) ([]row, []*ast.CommentGroup) {
	...
	if isLeafLitValue(value) {
		leafValueSlots = classifyComments(value)
	...
	var valDoc doc
	var attrsDoc doc
	bin, isBinChain := value.(*ast.BinaryExpr)
	...
	sel, isSelChain := value.(*ast.SelectorExpr)
	...
	case leafValueSlotsFiltered:
		valDoc = c.withCommentsSlots(value, c.exprCore(value), leafValueSlots)
...
func (c *converter) fieldKey(f *ast.Field) doc {
	key := c.label(f.Label)
	if q := internal.FieldQuantifier(f); q != nil {
		key = cat(key, c.typeParams(q, q.Lparen, q.Rparen, q.Params))
	}
	if f.Alias != nil {
...
// fieldValue is the expression following the colon. A shorthand universal's
// parameters are rendered with the field key, leaving its body as the value.
func fieldValue(f *ast.Field) ast.Expr {
	if q := internal.FieldQuantifier(f); q != nil {
		return q.Body
	}
	return f.Value
...
func (c *converter) fieldValDoc(f *ast.Field, preVal doc) doc {
	...
	// A shorthand quantifier also requires an expression after the colon;
	// its grammar does not accept a braceless field chain.
	valExprFun := c.fieldValExpr
	if len(f.Attrs) > 0 || internal.FieldQuantifier(f) != nil {
		valExprFun = c.expr
	}
	val := appendAttrs(valExprFun(value), f.Attrs)
	if preVal != nil {
		...
		if bracelessChainCollapsible(value) &&
			!HasDocComment(value) &&
			!valNeedsLeadingBreak(value) {
			val = cat(preVal, val)
		} else {
